...
#include "CppArrayObject.h"

void UCPPArrayObject::Enqueue_Implementation(int32 Data)
{
	IntArray.Emplace(Data);
	GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Cyan, FString::Printf(TEXT("Queued: %d"), Data));
}

int32 UCPPArrayObject::Dequeue_Implementation()
{
	int32 DequeueData = 0;
	if (!IntArray.IsEmpty())
	{
...
void UCPPArrayObject::IntPush_Implementation(const int32 Data)
{
	IntArray.Emplace(Data);
	GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Cyan, FString::Printf(TEXT("Pushed: %d"), Data));
}

int32 UCPPArrayObject::IntPop_Implementation()
{
	int32 PopData = 0;
	if (!IntArray.IsEmpty())
	{
...
int32 UCPPArrayObject::FibNumber(int n)
{
	if (n == 1 || n == 2)
		return 1;
